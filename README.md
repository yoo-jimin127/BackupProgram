...
<br>
   ...
   * 분기문으로 각 조건에 따라 다르게 처리 <br>
    * 인자 없는 경우 : ```getcwd()```로 현재 작업 경로 알아와서 그 아래에 ```mkdir``` <br>
    * 인자 2개 이상인 경우 : usage 출력 (argv 인덱스 개수 조건으로 걸어 printf("사용법 출력") <br>
    * 디렉토리 찾을 수 없는 경우 : *디렉토리 찾을 수 있는지 검사하는 함수 정의해 호출* <br>
    * 디렉토리 접근 권한 없는 경우 : *디렉토리의 접근 권한 확인하는 함수 정의해 호출 : (https://pliss.tistory.com/95)* <br>
