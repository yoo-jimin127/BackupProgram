...
void recoverFunc(LinkedList *linkedList, char *dirPath, char *fileName) {
	...
	else { //파일의 백업버전을 입력한 경우
		char recovered[256];
		memset(recovered, 0, 256);

		sprintf(chosen_time, "%s", rcvptr[userInput]); //사용자가 선택한 인덱스의 배열 가져옴
		printf("chosen_time : %s\n", chosen_time);
		char *token_tmp = strtok(chosen_time, " "); //순번을 읽어옴
		char *token_time = strtok(NULL, " ");//"순번. 백업시간"에서 공백자 기준 '백업시간'을 토큰으로 가져옴.
		printf("token_time : %s\n", token_time);

		sprintf(find_file, "%s_%s", fileName, token_time); //fileName.txt_210311~으로 저장됨.
		printf("find_file : %s\n", find_file);

		rename(recoverFile -> fileName, find_file);
		printf("recoverFile -> fileName : %s\n", recoverFile -> fileName);

		/*
		while((re_entry = readdir(re_dirptr)) != NULL) {
			char recovered[256];
			memset(d_name_of_file, 0, 256);
			memset(buf, 0, 256);
			memset(recovered, 0, 256);

			strcpy(d_name_of_file, re_entry -> d_name);
			printf("d_name_of_file : %s\n", d_name_of_file);
